...
def syn_inst_fitting():
    ...
    with model.fitter('inst') as ifit:
        ...
        ifit.set_labeling_strategy('CO2.ex', ['1'], [0.5], [0.997])
    ...
    pd.DataFrame(conc_cis, index = ['LB', 'UB']).T.to_excel(OUT_DIR+'/conc_le_CIs.xlsx')
...
def syn_inst_fitting_CIs():
    ...
    with model.fitter('inst') as ifit:
        ...
        ifit.prepare(n_jobs = 3)
        res = ifit.solve_with_confidence_intervals(solver = 'ralg', 
                                                   max_iters = 800, 
                                                   n_runs = 100, 
                                                   n_jobs = 30)
    ...
    conc_cis = res.estimate_confidence_intervals(which = 'conc', confidence_level = 0.95)
    pd.DataFrame(conc_cis, index = ['LB', 'UB']).T.to_excel(OUT_DIR+'/concentration_mc_CIs.xlsx')
